...
curl -X POST -H "Content-Type: application/json" -H "iPlanetDirectoryPro: ${ssoToken}" -d '{"username":"'${client}'", "AgentType": "OAuth2Client", "userpassword": "password", "com.forgerock.openam.oauth2provider.redirectionURIs": [], "com.forgerock.openam.oauth2provider.scopes": ["[0]=openid"], "com.forgerock.openam.oauth2provider.defaultScopes": [], "com.forgerock.openam.oauth2provider.tokenEndPointAuthMethod": "client_secret_post"}' "${openam}/json/agents?_action=create&_prettyPrint=true"
